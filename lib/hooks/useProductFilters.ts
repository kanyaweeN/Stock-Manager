...
import { matchesCatFilter } from "@/lib/core/cats";
import { itemTags, TAG_PRIORITY, type IngredientTag } from "@/lib/domain/ingredients";
import { buyTimes, isFrequent } from "@/lib/domain/price";
import { effectiveExpiry, needsAttention } from "@/lib/domain/expiry";
import { isLow, isOutOfStock } from "@/lib/domain/stock";
import { normalizeShopName, shopKey } from "@/lib/domain/orders";
...
export function useProductFilters(items: StockItem[], presets: string[]) {
  ...
  /** ร้านที่เลือกกรองอยู่ — เก็บเป็น `shopKey` ไม่ใช่ชื่อที่โชว์ (ดู lib/domain/orders.ts) */
  ...
  const sorters = useMemo(() => {
    ...
    // ของที่ไม่รู้วันหมดอายุไปท้ายสุดเสมอ (ไม่ใช่ "ยังอีกนาน" — เราแค่ไม่รู้ ดู lib/domain/expiry.ts)

...
import { applyCatEdit, type CatEditMode } from "@/lib/core/cats";
...
import type { ImportCandidate, PurchaseOrder, StockItem } from "@/lib/types";
...
export function useProductActions(setDb: (updater: (prev: StockDB) => StockDB) => void) {
  ...
  /** ลบ = ย้ายเข้าถังขยะ (กู้คืนได้ที่หน้าตั้งค่า) ไม่ใช่ลบถาวร — ดู lib/domain/trash.ts */
   ...
   * `replace` ทับทั้งหมด (พฤติกรรมเดิม) ตรรกะจริงอยู่ที่ `applyCatEdit` ใน lib/core/cats.ts
  ...
  /** ปัก/เอาดาวออก — เก็บเฉพาะตอนเป็นของโปรดจริงๆ (ดู normalizeItem ใน lib/db/normalize.ts) */
   ...
   * "ของจะหมดอีกกี่วัน" (ดู lib/domain/usage.ts) ตัว `qty` เฉยๆ บอกไม่ได้ว่าใช้เร็วแค่ไหน
   *
  ...
  const importOrder = (
    ...
    // วันที่ซื้อต้องอิงเวลาเครื่อง ไม่ใช่ UTC (ดู lib/core/date.ts) — `now` เป็น timestamp เต็มของ createdAt ไม่เกี่ยวกัน

 ...
 * ไว้ด้วยกัน (ร้านอื่นไม่ได้ห่อแบบนี้ จึงใช้ตัวแกะกลางใน `lib/orderPage.ts` แทน)
 *
 * ไฟล์นี้รู้แค่ "แถวไหนคือสินค้า" — ส่วนที่ทุกร้านเหมือนกัน (วันที่/ยอดเงิน/ชื่อร้าน/ส่วนผสม)
 * ถูกเตรียมมาให้แล้วโดย `extractOrderPage` และตรรกะที่ใช้ร่วมกันอยู่ที่ `lib/importText.ts`
 * (ข้อความ) + `lib/importDom.ts` (โครงหน้า) ส่วนป้ายภาษาไทยของยอดเงินอยู่ที่ `lib/importSites.ts`
 */
import { absUrl, getImgSrc, isProductImg, scanRowLeaves } from "./importDom";
import type { OrderRowContext } from "./importSites";
import { parseProductText } from "./importText";

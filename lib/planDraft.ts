 ...
 *
 * แยกออกจาก `components/PlanModal.tsx` เพื่อให้เทสต์ได้ เหมือน `lib/recipeDraft.ts`
 * **เพิ่มฟิลด์ใหม่ = แก้ `PlanDraft` + `toPlanDraft` + `fromPlanDraft` เท่านั้น**
 */
import { todayISO } from "./date";
import { priorityOf } from "./plan";
import type { PlanLine, PlanPriority, PurchasePlan } from "./types";
